...
jieba_data_path = os.path.dirname(__file__) + "/jieba_data/"
MATH_SEG = 'math_seg.txt'
jieba.load_userdict(str(jieba_data_path+MATH_SEG))
...
def jieba_cut(sentence, cut_all=False, HMM=True, use_paddle=False):
    return jieba.cut(sentence, cut_all=cut_all, HMM=HMM, use_paddle=use_paddle)


def load_math_keys():
    """加载数学关键词"""
    fin = open(jieba_data_path+'math_keywords_set.txt', encoding='utf-8')
...
def load_units():
    """加载单位"""
    fin = open(jieba_data_path+'units.txt', encoding='utf-8')
    units = fin.readlines()
    fin.close()
